...
use AcVendor\GuzzleHttp\Exception\ConnectException;
use AcVendor\GuzzleHttp\Exception\RequestException;
use AcVendor\GuzzleHttp\Promise as P;
use AcVendor\GuzzleHttp\Promise\FulfilledPromise;
use AcVendor\GuzzleHttp\Promise\PromiseInterface;
use AcVendor\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \AcVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \AcVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
